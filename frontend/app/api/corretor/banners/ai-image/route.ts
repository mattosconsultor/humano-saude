...
export async function POST(req: NextRequest) {
  try {
    const { imageBase64, operadora, plano, modalidade, angulo, template, ratio } = await req.json();
    ...
    for (const modelName of modelsToTry) {
      try {
        ...
        const prompt = `Você é um designer sênior de anúncios para Instagram no segmento de planos de saúde do Brasil.
